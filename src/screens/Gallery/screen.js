...
const Gallery = ({navigation}) => {
  ...
  useEffect(() => {
    getAllIndustry(), getJobType();
  }, []);
  useEffect(() => {
    getAllBanners();
  }, [IndustryType, isRefresh, selectedCityId, jobType]);
  const getAllBanners = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        // console.log('flatData', JSON.stringify(res.data));
        setFlatData(res.data);
      }
    } catch (e) {
          ...
          <View
            ...
            {flatData.length > 0 && (
              <Text
                ...
                {flatData?.length} {'Ergebnisse'}
              </Text>
            )}
            <View style={styles.boxMainView}>
              <TouchableOpacity
                style={[styles.sortTouch, {right: 10}]}
                onPress={() => {
                  OpenMenu();
                }}>
          ...
          <RBSheet
            ...
            <View
              ...
              <View style={{height: height * 0.6, marginHorizontal: 15}}>
                ...
                <Image
                  source={{uri: Globals.BASE_URL + zoomImage.path}}
                ...
                <View
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      downloadImage(zoomImage.fileName, zoomImage.path)
                    }
