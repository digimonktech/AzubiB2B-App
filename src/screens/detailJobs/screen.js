...
const DetailsJobs = ({ navigation, route }) => {
    ...
    const flatTasks = [
        { id: 1, txt: 'Support in the planning, implementation and maintenance of IT systems and networks' },
        { id: 2, txt: 'Error analysis and resolution of IT problems in close collaboration with the IT team' },
        { id: 3, txt: 'Installation, configuration and updating of hardware and software components' },
    ...
    const flatWish = [
        { id: 1, txt: 'High school diploma, technical college diploma or a good secondary school diploma with a strong interest in IT topics' },
        { id: 2, txt: 'Basic knowledge of operating systems (e.g. Windows, Linux) and network technologies' },
        { id: 3, txt: 'Willingness to learn, ability to work in a team and analytical thinking skills' },
    ...
    const flatOffer = [
        ...
        { id: 3, txt: 'Open communication and a successful and committed team that looks forward to seeing you' },
            ...
            <FlatList
                ...
                renderItem={({ item: imageItem }) => (
                    <TouchableOpacity
                        ...
                        onPress={() => { setDetailImage(imageItem?.filepath), setVisibleImage(true) }}>
    ...
    const renderDocs = ({ item }) => {
        return (
            <View style={{ width: width, flexDirection: 'row', paddingVertical: 10, alignSelf: 'center', alignItems: 'center', }}>
                <Image style={{ height: 20, width: 20, }} resizeMode='contain' source={require('../../assets/images/Frame.png')} />
                <Text style={{ color: '#000', fontSize: 14, fontFamily: fontFamily.poppinsSeBold, left: 5 }} onPress={() => { setDetailImage(item?.document?.filepath), setVisibleImage(true) }}>{item?.document?.fileName.slice(0, 10)}...</Text>
            </View>
        );
    };
        ...
        return (
            <View>
                <View style={[styles.renderMainView, { width: '93%', flex: 1 }]}>
                    ...
                    <TouchableOpacity style={{ width: '80%', paddingHorizontal: 10, paddingVertical: 10, }} activeOpacity={0.5} onPress={() => { getJobsDetails(item._id) }}>
                        <Text style={styles.nameTxt} numberOfLines={2}>{jobTitle}</Text>
                        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5, width: '85%' }}>
                            <View style={{ backgroundColor: '#fff', borderRadius: 5, height: 25, width: 25, alignItems: 'center', justifyContent: 'center' }}>
                                <Image style={{ height: '100%', width: '100%', borderRadius: 50 }} resizeMode='cover' source={{ uri: Globals.BASE_URL + item.companyLogo }} />
                            </View>
                        ...
                        <View style={styles.locView}>
                            ...
                            <View style={{ backgroundColor: '#007F9D', borderRadius: 2, height: 20, width: '25%', paddingHorizontal: 15, alignItems: 'center', justifyContent: 'center', left: 5 }}>
    ...
    const getRelatedJobs = async () => {
        try {
            setLoading(true);
            let res = await getApiCall({ url: 'employer/get-jobs-by-id?companyId=' + item?.companyId + '&skip=0&slectedCity=' + selectedCityId });
            if (res.status == 200) {
                    ...
                    {!loading &&
                        <TouchableHighlight underlayColor={'none'}>
                            <View style={[styles.renderMainView, { width: '93%', flex: 1 }]}>
                                ...
                                <View style={{ width: '100%' }}>
                                    <TouchableOpacity style={{ height: '50%', width: '20%', backgroundColor: '#0096A438', borderTopRightRadius: 10, borderBottomRightRadius: 10, justifyContent: 'center', alignItems: 'center', }} activeOpacity={0.5} onPress={() => { setVisibleApply(true) }}>
                                        <Image style={{ height: 20, width: 24, }} resizeMode='contain' source={require('../../assets/images/sms-tracking.png')} />
                                    </TouchableOpacity>
