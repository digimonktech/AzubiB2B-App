...
export const ModalSaveApply: React.FC<ModalApplyProps> = ({ visibleApply, setVisibleApply, applyData, deviceId }) => {
    ...
    const renderItemImage = (item: any) => {
        ...
        const removeImage = (uriToRemove: any) => {
            ...
            const updatedImageList = selectedImageShow.filter((image: any) => image.uri !== uriToRemove);
            const updatedbase64List = selectedImage.filter((image: any) => image.uri !== uriToRemove);
            setSelectedImageShow(updatedImageList);
            setSelectedImage(updatedbase64List);
        };
        return (
            <View style={{ height: 200, width: width / 3.38, paddingLeft: 5, marginTop: 20 }}>
                    ...
                    {showPrivacy ?
                        ...
                        <ScrollView showsVerticalScrollIndicator={true}>
                            ...
                            <View style={[styles.main, {
                                paddingBottom: (selectedImage.length > 3 && selectedImage.length <= 6) ?
                                    '75%' : selectedImage.length === 3 ? '60%' : '50%'
                            }]}>
                                <Text style={[styles.headingText, { fontSize: 15 }]}>{applyData?.jobTitle}</Text>
                                <Text style={[styles.headingText, { fontSize: 15, marginTop: 10, color: '#ff9046' }]}>{applyData?.company?.companyName}</Text>
                                {applyData.isDesktopView &&
                                    <TouchableOpacity
                                        onPress={() =>
                                            Linking.openURL(`https://www.azubiregional.de/jobs-board/${applyData._id}`)}
                                        style={{ marginTop: 10, backgroundColor: '#ff9046', padding: 10, width: '25%', alignItems: 'center' }}>
                                        <Text style={{ color: 'white', textDecorationLine: 'underline' }}>{'Job Link'}</Text>
                                    </TouchableOpacity>
                                }
                                <Text style={[styles.labelText, { marginTop: 10 }]}>{'Vorname/Nachname'}</Text>
                                <FormInput
                                    name="Name"
                                ...
                                <TouchableOpacity
                                    ...
                                    <Text style={{ fontFamily: fontFamily.poppinsSeBold, fontSize: 16, color: '#fff', left: 5 }}>{'Bewerbung absenden'}</Text>
                                </TouchableOpacity>
                            </View>
...
export const ModalApply: React.FC<ModalApplyProps> = ({ visibleApply, setVisibleApply, applyData, deviceId }) => {
    ...
    return (
        <Modal animationType="slide" visible={visibleApply} transparent={true}>
            <ScrollView contentContainerStyle={styles.modalBgView}>
                <View style={styles.modalMainView}>
                    <View style={[styles.flexView, { marginVertical: 10 }]}>
                        {showPrivacy ? <Text style={styles.headingText}>{'Datenschutz & AGB'}</Text> :
                            <Text style={styles.headingText}>{'Jetzt direkt bewerben'}</Text>
                        }
                        <TouchableOpacity onPress={() => handlePrivacyandclose()}>
                            <Image source={Images.modalClose} style={styles.closeImg} />
...
export const ModalAppointment: React.FC<ModalAppointmentProps> = ({ visibleAppointment, setVisibleAppointment, appointmentData }) => {
    const [content, setContent] = useState<any>([]);
    const [loading, setLoading] = useState(false);
    const [coverLabel, setCoverLabel] = useState<any>([]);
    const [showPrivacy, setShowPrivacy] = useState<any>(false);
    ...
    const takeAppointmentApi = async (values: IApply) => {
        ...
        const response = await networkWithoutToken.createMobileOtp().addAppointment(body);
    ...
    return (
        <Modal animationType="slide" visible={visibleAppointment} transparent={true}>
            <ScrollView contentContainerStyle={styles.modalBgView}>
                        ...
                        {showPrivacy ? <ScrollView>
                            ...
                            <ScrollView showsVerticalScrollIndicator={true}>
                                <View style={[styles.main, {
                                    ...
                                    <Text style={[styles.headingText, { fontSize: 15 }]}>{appointmentData.companyName}</Text>
                                    <Text style={[styles.labelText, { marginTop: 10 }]}>{'Vorname/Nachname'}</Text>
                                    <FormInput
                                        name="Name"
                                        placeholder={'Geben Sie Ihren Namen ein'}
                                    ...
                                    <TouchableOpacity
                                        ...
                                        <Text style={{ fontFamily: fontFamily.poppinsSeBold, fontSize: 16, color: '#fff', left: 5 }}>{'Bewerbung senden'}</Text>
                                    </TouchableOpacity>
                                </View>
                            </ScrollView>}
                    </View>
                        ...
                        <ScrollView>
                            <View style={{ flex: 1, width: '95%', alignSelf: 'center' }}>
                                <View style={styles.flexView}>
                                    <Text style={styles.headingText}>{'Direktbewerbung'}</Text>
                                    <TouchableOpacity onPress={() => { setSuccess(false), setVisibleAppointment(false) }}>
                                        <Image source={Images.modalClose} style={styles.closeImg} />
                                    </TouchableOpacity>
                                </View>
                                <View style={[styles.main, { alignSelf: 'center', alignItems: 'center', }]}>
                                    <Image source={require('../assets/images/Successfull.jpeg')} style={{ height: 200, width: 200, }} resizeMode="contain" />
                                    <Text style={[styles.headingText, { marginTop: 30 }]}>{'Glückwunsch!'}</Text>
                                    <Text style={styles.labelText}>{'Deine Nachricht wurde erfolgreich gesendet.'}</Text>
                                </View>
                                <Button
                                    bgColor={'#8C65A3'}
                                    _text={{ fontFamily: fontFamily.poppinsBold, fontWeight: 'bold' }}
                                    size={'lg'}
                                    onPress={() => { setSuccess(false), setVisibleAppointment(false) }}
                                    style={{ marginTop: 15, }}
                                    borderRadius={10}
...
export const ModalJobPic: React.FC<ModalJobDetailImageProps> = ({ visibleJobImage, setVisibleJobImage, imageData }) => {
    return (
        <Modal animationType="slide" visible={visibleJobImage} transparent={true}>
            <ScrollView contentContainerStyle={styles.modalBgView}>
                <View style={styles.modalMainView}>
                    <View style={[styles.flexView, { flexDirection: 'column', alignItems: 'flex-end' }]}>
                        {/* <Text style={styles.headingText}>{'Jetzt direkt bewerben'}</Text> */}
                    ...
                    <View style={{ height: 400, marginHorizontal: 15 }}>
                        <Image source={{ uri: Globals.BASE_URL + imageData }}
                            resizeMode="cover" borderRadius={10} style={{ height: 400, width: '100%' }} />
                    </View>
                </View>
            </ScrollView>
        </Modal>
...
export const ModalIndustry: React.FC<ModalIndustryProps> = ({ visibleIndustry, setVisibleIndustry }) => {
    return (
...
export const ModalLocation: React.FC<ModalLocationProps> = ({ visibleLocation, setVisibleLocation }) => {
                        ...
                        <View style={{ flex: 1 }}>
                            ...
                            <View style={styles.main}>
                                <FlatList
                                    ...
                                    renderItem={({ item }) => (
                                        <TouchableOpacity onPress={() => handleLocationSelect(item.name)}>
                                            <View style={styles.renderView}>
                                                <Text>{item.name}</Text>
                                            </View>
                                        </TouchableOpacity>
...
export const ModalSuccessfull: React.FC<ModalIndustryProps> = ({ visibleIndustry, setVisibleIndustry }) => {
    return (
        <Modal animationType="slide" visible={visibleIndustry} transparent={true}>
            <View style={styles.modalBgView}>
                <View style={styles.modalIndustryView}>
                    <ScrollView>
                        <View style={{ flex: 1 }}>
                            ...
                            <View style={styles.main}>
                                <Image source={require('../assets/images/correct.png')} style={{ height: 100, width: 100 }} resizeMode="contain" />
